...
<?php if (!$p['success']): ?>
    ...
    <?php elseif (!@$p[$type.'_db_initialized']): ?>
        <div class="row">
            <div class="col-md-8">
                The <?php echo $type ?> database has not yet been initialized. You can let the installer initialize it with the schema from <code>protected/data/<?php echo $type ?>/schema.<?php echo $p[$type.'_db_driver'] ?>.sql</code>.
            </div>
            <div class="col-md-4">
                <?php 
    ...
    <?php else: ?>
    <div class="row">
        <div class="col-md-8">There was an error during the database setup, please see the installer messages above for details.</div>
        ...
        echo CHtml::endForm(); ?></div>
    </div>
    <?php endif ?>
<?php elseif ($type == 'panel' && @$p['pw_issue']): ?>
    <div class="row">
        <div class="col-md-8">There seems to be an issue with the way passwords are stored in the "user" table of your panel database.</div>
        <div class="col-md-4"><?php 
