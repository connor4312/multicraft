...
// --------------
	...
	if ($ftp_rename_result == false) { 
		$errormessage = Yii::t('mc', 'Unable to rename <b>{a}</b> to <b>{b}</b>', array('{a}'=>$old, '{b}'=>$new));
		setErrorVars(false, $errormessage, debug_backtrace(), __FILE__, __LINE__);
		return false;
	}
...
// Print checkbox or hidden field
	if ($checkbox_hidden == "checkbox") {
        $visible = false;
        //if (isset($entry["selectable"]) && $entry["selectable"] == "ok") {
            $visible = true;
        //}
        
        echo "<input type=\"checkbox\" ".($visible ? '' : 'style="display: none"')." name=\"list[$number][dirfilename]\" id=\"list_" . $number . "_dirfilename\" value=\"" . $entry["dirfilename_html"] . "\" $onclick />\n";
	}
	else {
...
// -------------------------------------------------------------------------
	$content_type = 'application/force-download';//getContentType($filename);
